---
title: "MAJvsFJ_DGEseqandGSEAprep"
...
countdata <- as.matrix(read.csv("MAJvsFJ_INDV_comp_gene_count_matrix.csv", row.names="gene_id"))
dim(countdata)
head(countdata)

### Input the meta data or phenotype data
coldata <-(read.csv("MAJvsFJ_indv_comp_LIZARD_pheno_meta.csv", header=TRUE, row.names=1))
dim(coldata)
head(coldata)
...
dds$condition <- factor(dds$treatment, levels=c("AJ_Male","J_Female"))
