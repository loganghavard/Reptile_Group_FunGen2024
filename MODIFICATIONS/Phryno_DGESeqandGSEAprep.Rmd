...
countdata <- as.matrix(read.csv("Phrynosom_gene_count_matrix.csv", row.names="gene_id"))
dim(countdata)
head(countdata)
...
Anno <- read.csv("GSEA_and_count_names.csv", stringsAsFactors = FALSE, na.strings=c(""))
...
#for desert horn alt ref... i created a custom annotation type file containing GeneID and GeneName from BLAST results to identify HGCN symbols.
...
names(DGEresults)[1]<- "GeneID" 

#Merge anno with DGE results
DGE_Anno <- merge(Anno,DGEresults,by="GeneID",all.y = FALSE)
...
DGErank = subset(DGE_Anno_Rank, select = c(GeneName, rank) )
